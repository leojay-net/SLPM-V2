...
### FixedFloat Exchange Integration

For ZEC conversions, SLPM uses the FixedFloat exchange API:

| Route       | Source         | Destination    | Method                          |
| ----------- | -------------- | -------------- | ------------------------------- |
| ZEC to STRK | Zcash shielded | STRK           | ZEC -> BTC -> Lightning -> STRK |
| STRK to ZEC | STRK           | Zcash shielded | STRK -> Lightning -> BTC -> ZEC |

The exchange calls are made server-side to protect API credentials and provide retry logic.

---

## Zero-Knowledge Implementation

SLPM uses Noir circuits compiled to UltraHonk proofs, verified on-chain via Garaga. This provides cryptographic guarantees that no on-chain observer can link deposits to withdrawals.
...
Garaga provides efficient on-chain ZK proof verification using native Cairo circuits for elliptic curve operations:
...
```
    ...
    GAS EFFICIENCY: Garaga uses native Cairo circuits for EC operations,
                   achieving ~10x gas savings vs generic pairing verification
```
...
SLPM provides a standalone SDK for programmatic integration into applications.
...
For full SDK documentation, see the SDK README.
